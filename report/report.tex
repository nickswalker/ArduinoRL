...
\begin{document}
	...
	It is possible to implement the update using only $n$ additional space, where $n$ is the number of weights, but this is easy to do incorrectly. If the action selection step is placed after the memory allocation, the stack will consume $2n$ memory; maximizing the value function over possible next states requires an additional $n$ stack space.
		...
		\begin{algorithm}
			...
			\begin{algorithmic}[1] % The number tells where the line numbering should start
				...
				\State $\bm{x} \gets (\alpha \space a)\bm{x}$ \Comment $\bm{x}$ is now weight update
	...
	I used a simple discretization of the state space. The range of each joint is divided into 8\space\space20\degree\space sections. The two sets of section features along with a binary feature for the LED state are then gridded, resulting in 128 mutually exclusive binary features. 
